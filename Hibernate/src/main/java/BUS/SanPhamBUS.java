...
public class SanPhamBUS {
    ...
    public boolean xoaSanPham(String ma) throws InstantiationException, IllegalAccessException, UnsupportedLookAndFeelException {
        if (ma.trim().equals("")) {
            new MyDialog("Chưa ch�?n sản phẩm để xoá!", MyDialog.ERROR_DIALOG);
            return false;
        }
    ...
    public boolean suaSanPham(String ma,
        ...
        try {
            if (ma.trim().equals("")) {
                new MyDialog("Chưa ch�?n sản phẩm để sửa!", MyDialog.ERROR_DIALOG);
                return false;
            }
            donGia = donGia.replace(",", "");
            int maSP = Integer.parseInt(ma);
            ...
            if (maLoai == 0) {
                new MyDialog("Vui lòng ch�?n Loại sản phẩm!", MyDialog.ERROR_DIALOG);
                return false;
            }
            ...
            if (donViTinh.trim().equals("")) {
                new MyDialog("Vui lòng đi�?n �?ơn vị tính!", MyDialog.ERROR_DIALOG);
                return false;
            }
            ...
            new MyDialog("Nhập số hợp lệ cho �?ơn giá và Số lượng!", MyDialog.ERROR_DIALOG);
        }
        return false;
    }
